...
public class InstanceTypeToHardware implements Function<InstanceType, Hardware> {
   ...
   public Hardware apply(InstanceType input) {
      HardwareBuilder builder = new HardwareBuilder().ids(input.instanceTypeId()).name(input.instanceTypeId())
            .hypervisor("none").processors(getProcessors(input.cpuCoreCount())).ram(input.memorySize().intValue());
