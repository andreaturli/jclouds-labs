...
public class SecurityGroupApiMockTest extends BaseECSComputeServiceApiMockTest {
   ...
   public void testListSecurityGroups() throws InterruptedException {
      ...
      assertSent(server, "GET", "DescribeSecurityGroups", 2);
   }

   public void testListSecurityGroupsReturns404() {
   ...
   public void testListSecurityGroupsWithOptions() throws InterruptedException {
      ...
      IterableWithMarker<SecurityGroup> securitygroups = api.securityGroupApi().list(Regions.EU_CENTRAL_1.getName(), ListSecurityGroupsOptions.Builder
              .paginationOptions(PaginationOptions.Builder.pageNumber(1).pageSize(5)));

      assertEquals(size(securitygroups), 5);
      assertEquals(server.getRequestCount(), 1);

      assertSent(server, "GET", "DescribeSecurityGroups", 1);
   }
