...
public class VSwitchApiLiveTest extends BaseECSComputeServiceApiLiveTest {
   ...
   public void tearDown() {
      if (vSwitchId != null) {
         assertNotNull(api().delete(Regions.EU_CENTRAL_1.getName(), vSwitchId));
      }
      if (vpcId != null) {
         assertNotNull(api.vpcApi().delete(Regions.EU_CENTRAL_1.getName(), vpcId));
      }
   }
   ...
   public void testList() {
      ...
      assertTrue(Iterables.all(api().list(Regions.EU_CENTRAL_1.getName()).concat(), new Predicate<VSwitch>() {
         ...
         public boolean apply(VSwitch input) {
            ...
            return !isNullOrEmpty(input.vSwitchId());
         }
