...
public class VPCApiMockTest extends BaseECSComputeServiceApiMockTest {
   ...
   public void testListVPCs() throws InterruptedException {
      ...
      Iterable<VPC> vpcs = api.vpcApi().list(Regions.EU_CENTRAL_1.getName()).concat();
   ...
   public void testListVPCsWithOptionsReturns404() throws InterruptedException {
      server.enqueue(response404());
      IterableWithMarker<VPC> vpcs = api.vpcApi().list(Regions.EU_CENTRAL_1.getName(), paginationOptions(pageNumber(2)));
      assertTrue(isEmpty(vpcs));
      assertEquals(server.getRequestCount(), 1);
      assertSent(server, "GET", "DescribeVpcs", ImmutableMap.of("RegionId", Regions.EU_CENTRAL_1.getName()), 2);
   }
