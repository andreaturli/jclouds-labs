...
public class TagApiLiveTest extends BaseECSComputeServiceApiLiveTest {
   ...
   public void setUp() {
      ...
      Request request = api().add(Regions.EU_CENTRAL_1.getName(), securityGroupId, RESOURCE_TYPE,
              TagOptions.Builder.tag1Key("owner").tag1Value("jclouds"));
   ...
   public void testList() {
      ...
      assertTrue(Iterables.all(api().list(Regions.EU_CENTRAL_1.getName()).concat(), new Predicate<Tag>() {
         @Override
         public boolean apply(Tag input) {
            found.incrementAndGet();
            return !isNullOrEmpty(input.tagKey()) && !isNullOrEmpty(input.tagValue());
         }
      }), "All tags must have the 'key' and 'value' fields populated");
